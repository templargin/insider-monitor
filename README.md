...
- **…but only a *transient* failure suppresses a page.** `DataUnavailable` carries a `transient` flag and the outage guard counts only those. Unreachable upstreams are transient (a `companyfacts` 429, a throttled price lookup for a real symbol); structural gaps are not — a closed-end fund with no companyfacts at all (BXSY), an IFRS/20-F filer with no us-gaap balance sheet (BWMX), a share count predating its own balance sheet, or a Form 4 whose ticker field is the literal string `NONE` (non-traded BDCs — these route to the no-ticker path via `_is_real_ticker`). A day whose entire candidate pool is permanently unevaluable is a **quiet day**, and its page is written **empty**. *(2026-07-30 went dark on exactly this: four candidates, all four structurally unevaluable, read as "SEC is down".)*
