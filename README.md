...
- **Gross Profit fallback** for filers that report Operating Income and G&A but no CoR/COGS tag (PFHO, AXR, TOI, BDL, VTS, CURI, NXDT, GROW, TXO, plus insurers HRTG/GSHD that don't gate out): GP = OpInc + G&A, i.e., assume G&A is the only non-COGS operating expense. Gated to skip biotechs (`ResearchAndDevelopmentExpense` tagged) and banks (`NoninterestExpense` tagged) where the assumption produces nonsense; also requires Rev ≥ $1M so pre-revenue filers don't get -1000% margins. Periods filled via this fallback have their Operating Expense row suppressed (it would otherwise duplicate SG&A). Restaurants and insurance carriers get visibly off numbers (BDL ~7% because Labor isn't tagged as CoR; HRTG ~44% because the formula isn't underwriting margin) — accepted for now; archetype-specific derivations are a follow-up.
