...
2. **Classified by the us-gaap debt hierarchy, not a tag list.** Borrowings are recognized by family pattern (`LineOfCredit`, `SeniorNotes`, `ConvertibleNotesPayable`, `LongTermLineOfCredit`, finance leases, …) and deduped by *tier* — a reported subtotal (`LongTermDebtNoncurrent`) subsumes the instruments beneath it, so AVD's `LongTermDebtNoncurrent` **$264M** + `LineOfCredit` **$283M** doesn't double to $547M. This captured debt the ladder reported as **$0**: REI's **$426M** revolver, VTS's **$144M**, AFCG's **$76M** senior notes, OPAD's notes/loans, XRN's **$663M** (whose true EV crosses the $1B screen). Asset look-alikes are excluded — notes/loans *receivable* (DAVE/BNPL loan books), AFS debt *securities*, FHLB *stock*, `*Gross` duplicates.
3. **Flag, never plug** (move 3). The result is reconciled against the filer's reported total liabilities; residual uncertainty is surfaced on the page (`valuation.debt_flag`, rendered as a ⚠ + warning) instead of being absorbed into an "Other" plug: `debt_tags_overlap_clamped` (raw sum exceeded total liabilities → overlapping tags double-counted; falls back to the largest single instrument that fits, never a plug of unclassified liabilities — OPAD, BZUN), `unexplained_liabilities` (a material slice of liabilities is neither recognized debt nor a named non-debt item — this is what catches debt under a filer's *custom namespace*, which companyfacts can't carry), `financial_institution` (deposits are a large share of the balance sheet → bank/thrift, EV isn't a meaningful metric). Across the stored universe: 196 audited, 89 debt values corrected, 42 flagged, 0 reads exceeding total liabilities.
