...
| A ticker on a daily page links to a 404 company page | The company refresh failed after the row was written (SPRU 2026-09-24: SEC 503), or the filer typed a lowercase symbol (CV 2026-09-21, `cv`) | Automatic since 2026-09-24: `daily_run` calls `pipeline.heal_missing_companies()` after today's work, and symbols are upper-cased at parse. Grep the run log for `still missing:`. Manual equivalent from a **synced** clone: `./venv/bin/python -c "from scraper import pipeline; print(pipeline.heal_missing_companies())"` then `build_site` |
