...
# Every test that can reach the 540-day staleness bound MUST pin `today`. Two of
# these originally let it default to date.today() against a hardcoded 12/31/25
# period, so they would have started failing on 2027-06-25 — and daily.yml gates
# `scripts.daily_run` on pytest, so the site would have silently stopped updating
# on that date, for a reason that looks nothing like a test.
NOW = date(2026, 7, 15)
...
def test_sums_four_quarters():
    ...
    assert ltm_revenue(fins(quarterly=q), today=NOW) == (1_507_619_000, "3/31/26")
...
def test_annual_fallback_for_filers_with_no_quarterly_xbrl():
    ...
    assert ltm_revenue(fins(annual=a), today=NOW) == (606_792_000, "12/31/25")
...
def test_quarterly_wins_over_annual():
    ...
    assert ltm_revenue(fins(quarterly=q, annual=a), today=NOW)[0] == 40


def test_annual_skips_an_empty_leading_column():
    a = grid(["Total Revenue"], [[None, 250_000]], ["LTM", "12/31/25"])
    assert ltm_revenue(fins(annual=a), today=NOW) == (250_000, "12/31/25")
...
def test_stale_annual_revenue_does_not_count():
    ...
    assert ltm_revenue(fins(annual=a), today=NOW) == (0.0, None)


def test_recent_annual_still_counts():
    a = grid(["Total Revenue"], [[606_792_000]], ["12/31/25"])
    assert ltm_revenue(fins(annual=a), today=NOW)[0] == 606_792_000
...
def test_no_revenue_row_anywhere_is_a_real_zero():
    ...
    assert ltm_revenue(fins(quarterly=q), today=NOW) == (0.0, None)
...
def test_partial_quarters_sum_what_exists():
    ...
    assert ltm_revenue(fins(quarterly=q), today=NOW)[0] == 10


def test_missing_statements_are_the_callers_problem():
    assert ltm_revenue(None, today=NOW) == (0.0, None)
